...
public class TestDFSIO implements Tool {
    ...
    private static String getBaseIODataDir(Configuration conf) {
        return conf.get("test.build.iodata", "/benchmarks/TestDFSIO");
    }
    ...
    private static Path getDataDir(Configuration conf) {
        return new Path(getBaseIODataDir(conf), "io_data");
    }
     ...
     * and reads bufferSize bytes starting at the chosen position. It stops
     * after reading the totalSize bytes, specified by -size.
     *
     * There are three type of reads. 1) Random read always chooses a random
     * position to read from: skipSize = 0 2) Backward read reads file in
     * reverse order : skipSize < 0
    ...
    public static class RandomReadMapper extends IOStatMapper {
        ...
        /**
         * Get next offset for reading. If current < 0 then choose initial
         * offset according to the read type.
         *
    ...
    public int run(String[] args) throws IOException {
        ...
        LOG.info("baseIODir = " + getBaseIODataDir(config));
    ...
    private void cleanup(FileSystem fs)
        ...
        fs.delete(new Path(getBaseIODataDir(config)), true);
    }
}
